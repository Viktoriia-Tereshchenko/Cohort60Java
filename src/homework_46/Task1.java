...
public class Task1 {
    ...
    public static Map<String, Integer> intersection(Map<String, Integer> map1, Map<String, Integer> map2) {
        ...
        Predicate<String> predicate = key -> map1.containsKey(key) && map2.containsKey(key);
        Consumer<String> consumer = key -> result.put(key, map1.get(key));

        map1.forEach((key, value) -> {
            if (predicate.test(key)) consumer.accept(key);
        });
        return result;
    }
}
