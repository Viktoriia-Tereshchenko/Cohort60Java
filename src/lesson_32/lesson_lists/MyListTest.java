package lesson_32.lesson_lists;

import lists.MyArrayList;
import lists.MyList;

public class MyListTest {
    public static void main(String[] args) {

        // создаем объект типа ИНТЕРФЕЙС, а не класс!

        MyList<Integer> integerList = new MyArrayList<>();

        integerList.add(10);
        System.out.println(integerList);

        integerList.addAll(120, 250, 1000, -100, -200, 0);
        System.out.println(integerList);

        Integer value = integerList.get(0);
        System.out.println(value);

        System.out.println("integerList.size(): " + integerList.size());

        int index = integerList.indexOf(120);
        System.out.println("integerList.indexOf(120): " + index);
        System.out.println("integerList.indexOf(250): " + integerList.indexOf(250));

        // удаоить элемент со значение 0
        System.out.println("\n=======================");
        System.out.println(integerList);
        System.out.println(integerList.remove(Integer.valueOf(0))); // принудительная упаковка
        System.out.println(integerList);


        integerList.add(null);
        System.out.println(integerList);

        // NPE из-за добавление null в качестве значения
        System.out.println(integerList.lastIndexOf(1000));

    }

}
